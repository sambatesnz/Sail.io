...
final class BoatInfo {
     ...
     */
    static Stack<GenericBoat> createBoats(int lowestSourceId, int amount) {
        int sourceId = lowestSourceId + amount-1;
        Map<String, List<String>> dataMap = getBoatMap();
        Stack<GenericBoat> boats = new Stack();
        ...
        for (int i = amount-1; i>-1 ; i--) { //So we put nice boat names on top
            ...
            GenericBoat boat = new Boat(boatName, boatData.get(1), sourceId, boatData.get(0));
            boat.setHeading(180);
